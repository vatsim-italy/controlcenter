                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($training->evaluations->count() > 0)
                                    @php
                                        // evaluations sorted by their 'date' field (not created_at)
                                        $evalDate = Carbon\Carbon::make(
                                            $training->evaluations->sortBy('date')->last()->date
                                        );

                                        $trainingIntervalExceeded = $evalDate->diffInDays() >= Setting::get('trainingInterval');
                                    @endphp

                                    <span title="{{ $evalDate->toEuropeanDate() }}">
                                        @if($evalDate->isToday())
                                            <span class="{{ ($trainingIntervalExceeded && $training->status != \App\Helpers\TrainingStatus::AWAITING_EXAM->value && !$training->paused_at) ? 'text-danger' : '' }}">
                                                Today
                                            </span>
                                        @elseif($evalDate->isYesterday())
                                            <span class="{{ ($trainingIntervalExceeded && $training->status != \App\Helpers\TrainingStatus::AWAITING_EXAM->value && !$training->paused_at) ? 'text-danger' : '' }}">
                                                Yesterday
                                            </span>
                                        @elseif($evalDate->diffInDays() <= 7)
                                            <span class="{{ ($trainingIntervalExceeded && $training->status != \App\Helpers\TrainingStatus::AWAITING_EXAM->value && !$training->paused_at) ? 'text-danger' : '' }}">
                                                {{ $evalDate->diffForHumans(['parts' => 1]) }}
                                            </span>
                                        @else
                                            <span class="{{ ($trainingIntervalExceeded && $training->status != \App\Helpers\TrainingStatus::AWAITING_EXAM->value && !$training->paused_at) ? 'text-danger' : '' }}">
                                               {{ $evalDate->diffForHumans(['parts' => 2]) }}
                                           </span>
                                        @endif
                                    </span>
                                @else
                    ...
                    <table class="table table-striped table-hover table-leftpadded mb-0">
                        ...
                        <tbody>
                            @foreach($eligibilities as $eligibility)
                                @if($eligibility['rating']['endorsement_type'] === 'T1' and $eligibility['reason'] !== "Endorsement already held")
                                    <tr>
